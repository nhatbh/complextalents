...
public class WeaponMasteryManager implements ResourceManagerReloadListener {
        ...
        try (InputStream is = WeaponMasteryManager.class.getResourceAsStream("/data/complextalents/weapon_data.json")) {
            if (is != null) {
                try (InputStreamReader reader = new InputStreamReader(is, StandardCharsets.UTF_8)) {
                    ...
                    LOGGER.info("Successfully loaded {} weapon mappings from classpath resource.", weaponToPathMap.size());
                }
            } else {
    ...
    public int getRequiredPlayerLevelForTier(int targetLevel) {
        if (targetLevel <= 2) return 1;    // Novice
        if (targetLevel <= 5) return 10;   // Apprentice
        if (targetLevel <= 9) return 20;   // Adept
        if (targetLevel <= 14) return 30;  // Expert
     ...
     * Allows third-party mods to assign items to Weapon Master paths programmatically.
     *
     * @param itemId ResourceLocation of the item (e.g. "my_mod:epic_sword")
    ...
    public void registerWeaponMapping(ResourceLocation itemId, WeaponPath path, int requiredRankLevel) {
        if (itemId != null && path != null) {
            ...
            LOGGER.info("Registered API weapon mapping for '{}' -> Path: {}, RequiredRank: {}", itemId, path, requiredRankLevel);
        }
    }
    ...
    public static double getADBonusMultiplier(int cumulativeLevel) {
        double[] bonuses = {
            ...
            0.22, // Lv 4: Tier 2 Max (+2) (+22%)
            0.30, // Lv 5: Tier 3 (+1) (+30%)
            0.40, // Lv 6: Tier 3 (+2) (+40%)
            0.52, // Lv 7: Tier 3 Max (+3) (+52%)
            0.65, // Lv 8: Tier 4 (+1) (+65%)
            0.80, // Lv 9: Tier 4 (+2) (+80%)
            1.00, // Lv 10: Tier 4 (+3) (+100%)
            1.25, // Lv 11: Tier 4 Max (+4) (+125%)
            1.55, // Lv 12: Tier 5 (+1) (+155%)
            1.90, // Lv 13: Tier 5 (+2) (+190%)
            2.30, // Lv 14: Tier 5 (+3) (+230%)
            2.80, // Lv 15: Tier 5 (+4) (+280%)
            3.40, // Lv 16: Tier 5 (+5) (+340%)
            4.10, // Lv 17: Tier 5 (+6) (+410%)
            4.90, // Lv 18: Tier 5 (+7) (+490%)
            5.80, // Lv 19: Tier 5 (+8) (+580%)
            6.80  // Lv 20: Tier 5 Max (+9) (+680%)
        };
    ...
    public static int getRequiredMasteryLevel(int currentTier, int refineInTier) {
        return switch (currentTier) {
            ...
            case 3 -> {
                if (refineInTier <= 1) yield 5;
                if (refineInTier == 2) yield 6;
                yield 7;
            }
            case 4 -> {
                if (refineInTier <= 1) yield 9;
                if (refineInTier == 2) yield 10;
                if (refineInTier == 3) yield 11;
                yield 12;
            }
            case 5 -> 14;
            default -> 0;
    ...
    public static String getRankNameForLevel(int level) {
        if (level <= 1) return "Novice [L" + (level + 1) + "]";
        if (level <= 4) return "Apprentice [L" + (level - 1) + "]";
        if (level <= 8) return "Adept [L" + (level - 4) + "]";
        if (level <= 13) return "Expert [L" + (level - 8) + "]";
        return "Master [Pinnacle]";
    }

    /**
     * Rolls a randomized total refine count for loot weapons with UNLOCKED tier ascension.
     * Probability rule:
     * - Base (+0): 75%
     * - Refine +1: 75% of 25% = 18.75%
     * - Refine +2: 75% of 6.25% = 4.6875%
     * - Refine +3: 75% of 1.5625% = 1.171875% (Ascends tier!)
     * ...with an astronomical chance to ascend up to Master Tier 5 Pinnacle (Cumulative Lv 20).
     */
    ...
    /**
     * Rolls a randomized refine damage variance between -20% and +20% (-0.20 to +0.20).
     * Uses a Gaussian normal distribution so probability drops exponentially toward the edges (-20% and +20%).
     */
    /**
    ...
    /**
     * Gets the total cumulative XP required to reach a specific refine rank (0 to 20).
     */
     ...
     * Always accounts for the starting weapon tier baseline XP as the minimum default.
    ...
    public static float rollExponentialRefineVariance(net.minecraft.util.RandomSource random) {
        double gaussian = random.nextGaussian();
        double variance = gaussian * (0.20 / 2.5);
        return (float) Math.max(-0.20, Math.min(0.20, variance));
    }

    public static java.util.UUID getOrCreateRefineSeed(net.minecraft.world.item.ItemStack stack) {
        if (stack == null || stack.isEmpty()) return java.util.UUID.randomUUID();
    ...
    public static float rollRefineVarianceForRank(net.minecraft.world.item.ItemStack stack, int rank) {
        java.util.UUID seedUuid = getOrCreateRefineSeed(stack);
        long seed = seedUuid.getLeastSignificantBits() ^ ((long) rank * 0x9E3779B97F4A7C15L);
        net.minecraft.util.RandomSource random = net.minecraft.util.RandomSource.create(seed);
        return rollExponentialRefineVariance(random);
    }

    /**
     * Gets the effective AD bonus multiplier for an item stack using a unified level pipeline.
     * Default base tier levels (1 to baseCumulativeLevel) accumulate level increments at guaranteed 0% variance.
     * Manual enhancement refine levels (above baseCumulativeLevel) accumulate level increments with per-level random quality variance.
     */
    public static double getADBonusMultiplier(net.minecraft.world.item.ItemStack stack) {
        if (stack == null || stack.isEmpty()) return 0.0;
        int startingTier = getInstance().getWeaponTier(stack);
        if (startingTier <= 0) return 0.0;
        ...
        net.minecraft.nbt.ListTag varianceList = (stack.hasTag() && stack.getTag().contains("RefineVariances", net.minecraft.nbt.Tag.TAG_LIST)) 
                ? stack.getTag().getList("RefineVariances", net.minecraft.nbt.Tag.TAG_FLOAT) 
                : new net.minecraft.nbt.ListTag();

        double totalBonus = 0.0;

        // Single unified level pipeline from level 1 to targetCumulativeLevel
        for (int level = 1; level <= targetCumulativeLevel; level++) {
            double baseBonusCurr = getADBonusMultiplier(level);
            double baseBonusPrev = getADBonusMultiplier(level - 1);
            double increment = baseBonusCurr - baseBonusPrev;

            float v = 0.0f;
            // Default base tier levels (level <= baseCumulativeLevel) have 0 variance (v = 0.0)
            // Manual enhance levels (level > baseCumulativeLevel) roll/apply random variance v
            if (level > baseCumulativeLevel) {
                int manualStepIndex = level - baseCumulativeLevel - 1;
                if (manualStepIndex < varianceList.size()) {
                    v = varianceList.getFloat(manualStepIndex);
                    v = Math.max(-0.20f, Math.min(0.20f, v));
                } else {
                    v = rollRefineVarianceForRank(stack, level - baseCumulativeLevel);
                }
            }

            totalBonus += increment * (1.0 + v);
        }
    ...
    /**
     * Applies randomized refinement to a loot weapon stack.
     * If refined (rolledRank > 0), restores durability, adds Unbreakable status, sets cumulative RefineXP,
     * and rolls per-level quality variances for manual refine ranks.
     */
    public static net.minecraft.world.item.ItemStack applyRandomRefinementForLoot(net.minecraft.world.item.ItemStack stack, net.minecraft.util.RandomSource random) {
        if (stack == null || stack.isEmpty()) return stack;

        int startingTier = getInstance().getWeaponTier(stack);
        if (startingTier <= 0) {
            return stack; // Not a mastery weapon
        }

        if (stack.hasTag() && stack.getTag().contains("RefineRank")) {
            return stack; // Already has refine rank set
        }
        ...
        if (rolledRank > 0) {
            ...
            net.minecraft.nbt.ListTag varianceList = new net.minecraft.nbt.ListTag();
            for (int r = 1; r <= rolledRank; r++) {
                varianceList.add(net.minecraft.nbt.FloatTag.valueOf(rollRefineVarianceForRank(stack, r)));
            }
